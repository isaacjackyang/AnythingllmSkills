...
import { consumeConfirmToken, createPendingAction, decidePendingAction, listPendingActions, markPendingActionExecuted, resetPendingActionsDbForTest, } from "../approvals_store.ts";
...
function makeProposal(risk: "low" | "medium" | "high"): string {
    return String({
        trace_id: "trace-1",
        type: "tool_proposal",
        tool: "run_job",
...
test("confirm-only action executes after token consume", async (): Promise<string> => {
...
test("double-confirm action rejects token before approval", async (): Promise<string> => {
    await resetPendingActionsDbForTest();
    const action = await createPendingAction({
        type: "approval",
