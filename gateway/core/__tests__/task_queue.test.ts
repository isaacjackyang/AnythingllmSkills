...
import { cancelTask, claimNextTask, completeTask, deleteTask, enqueueTask, failTask, listTasks, resetTaskDbForTest, } from "../tasks/store.ts";
test("enqueue persists and claim follows priority order", async (): Promise<string> => {
...
test("failed running task moves to retry_scheduled then failed after max attempts", async (): Promise<string> => {
    await resetTaskDbForTest();
    ...
    const nextRun = await claimNextTask("worker-b", new Date(Date.now() + 120000));
...
test("complete/cancel/delete obey state constraints", async (): Promise<string> => {
    await resetTaskDbForTest();
...
test("listTasks can filter by agent_id", async (): Promise<string> => {
    await resetTaskDbForTest();
