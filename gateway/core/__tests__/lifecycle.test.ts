...
import { __setLastBeatEpochMsForTest, getLifecycleSnapshot, parseHeartbeatInterval, startHeartbeat, stopHeartbeat, updateSoul, } from "../lifecycle.ts";
test("parseHeartbeatInterval falls back for invalid values", (): string => {
...
test("startHeartbeat emits beats and exposes soul data", async (): Promise<string> => {
    stopHeartbeat();
    startHeartbeat(20);
...
test("snapshot becomes stale when last beat exceeds threshold", (): string => {
    stopHeartbeat();
    startHeartbeat(50);
    const now = Date.now();
...
test("soul can be updated with revision increment", async (): Promise<string> => {
