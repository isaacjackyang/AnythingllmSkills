...
async function readDb(): Promise<PendingActionDb> {
  try {
...
async function writeDb(db: PendingActionDb): Promise<void> {
  await ensureDbPath();
...
function isExpired(action: PendingAction, now = new Date()): boolean {
  return ["pending", "approved"].includes(action.status) && action.expires_at <= now.toISOString();
}

function normalizeStatus(action: PendingAction, now = new Date()): PendingAction {
...
export async function createPendingAction(input: {
  ...
  return withLock(async () => {
    ...
    const now = new Date();
    const ttlMs = Math.max(1_000, Math.trunc(input.ttl_ms ?? 15 * 60_000));
...
export async function listPendingActions(filters: { status?: PendingActionStatus; type?: PendingActionType; limit?: number } = {}): Promise<PendingAction[]> {
  return withLock(async () => {
    const now = new Date();
    const db = await readDb();
    let changed = false;
    db.actions = db.actions.map((item) => {
      ...
      if (normalized.status !== item.status) changed = true;
      return normalized;
    });
...
export async function getPendingActionById(actionId: string): Promise<PendingAction | undefined> {
  return withLock(async () => {
    const db = await readDb();
...
export async function consumeConfirmToken(confirmToken: string, actorId: string): Promise<PendingAction | undefined> {
  return withLock(async () => {
    const db = await readDb();
    const now = nowIso();
    const item = db.actions.find((row) => row.confirm_token === confirmToken);
    if (!item) return undefined;
    if (!item.requires_confirm_token) return undefined;
    if (item.requires_approval && item.status !== "approved") return undefined;
    if (!item.requires_approval && item.status !== "pending") return undefined;
...
export async function decidePendingAction(actionId: string, actorId: string, decision: "approve" | "reject", reason?: string): Promise<PendingAction> {
  return withLock(async () => {
    const db = await readDb();
    const item = db.actions.find((row) => row.id === actionId);
    if (!item) throw new Error("pending action not found");
    if (item.status !== "pending") throw new Error("pending action is not pending");
    const now = nowIso();
    if (item.expires_at <= now) {
      item.status = "expired";
      item.updated_at = now;
...
export async function markPendingActionExecuted(actionId: string): Promise<PendingAction> {
  return withLock(async () => {
    const db = await readDb();
    const item = db.actions.find((row) => row.id === actionId);
    if (!item) throw new Error("pending action not found");
