...
export function inspectProposalIntent(proposal: ToolProposal): ProposalIntent {
  ...
  if (deleteHit) evidence.push(`delete:${deleteHit}`);
  if (formatHit) evidence.push(`format:${formatHit}`);
