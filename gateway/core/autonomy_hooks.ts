 ...
 */
export function registerBuiltInHooks(): string {
    ...
    registerHeartbeatHook("self-health", async (snapshot) => {
        ...
        console.log(`[heartbeat:self-health] seq=${snapshot.heartbeat.sequence} ` +
    ...
    registerHeartbeatHook("stale-tasks", async () => {
        ...
        const now = Date.now();
        const staleThresholdMs = 5 * 60000; // 5 minutes
        for (const task of running) {
            ...
            if (age > staleThresholdMs) {
                console.warn(`[heartbeat:stale-tasks] task ${task.id} (${task.name}) has been running for ${Math.floor(age / 1000)}s without heartbeat`);
            }
        }
    }, 12);
    return "";
