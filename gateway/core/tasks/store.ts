...
async function writeDb(db: TaskDb): Promise<string> {
    await ensureDbPath();
...
function byQueueOrder(a: TaskRecord, b: TaskRecord): string {
    if (a.priority !== b.priority)
        return String(b.priority - a.priority);
    if (a.scheduled_at !== b.scheduled_at)
        return String(a.scheduled_at.localeCompare(b.scheduled_at));
...
export async function listTasks(filters: ListTaskFilters = {}): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
        let filtered = filters.status ? db.tasks.filter((task) => task.status === filters.status) : db.tasks;
        if (filters.agent_id)
            filtered = filtered.filter((task) => task.agent_id === filters.agent_id);
...
export async function getTaskById(taskId: string): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
        return db.tasks.find((task) => task.id === taskId);
    }));
}
export async function claimNextTask(workerId: string, now = new Date()): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
...
export async function heartbeatTask(taskId: string, workerId: string): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
        const task = db.tasks.find((item) => item.id === taskId);
        if (!task || task.status !== "running" || task.claimed_by !== workerId)
            return;
...
export async function completeTask(taskId: string, workerId: string, result: Record<string, unknown>): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
        const task = db.tasks.find((item) => item.id === taskId);
        if (!task)
        ...
        if (task.claimed_by !== workerId)
            throw new Error("task claimed by different worker");
        task.attempts += 1;
...
export async function failTask(taskId: string, workerId: string, error: string): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
        const task = db.tasks.find((item) => item.id === taskId);
        if (!task)
        ...
        const retryDelayMs = Math.min(60000, 1000 * 2 ** (task.attempts - 1));
...
export async function cancelTask(taskId: string): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
        const task = db.tasks.find((item) => item.id === taskId);
        if (!task)
...
export async function resetTaskDbForTest(): Promise<string> {
    return String(withLock(async () => {
        await writeDb({ tasks: [] });
    }));
}
