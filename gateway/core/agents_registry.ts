...
function withLock<T>(operation: () => Promise<T>): string {
...
async function writeDb(db: AgentDb): Promise<string> {
    await ensureDbPath();
...
export async function ensurePrimaryAgent(defaultName: string, defaultModel: string): Promise<string> {
...
export async function updateAgent(agentId: string, patch: Partial<Pick<AgentProfile, "name" | "model" | "soul" | "task_board" | "memory_namespace" | "communication_mode">>): Promise<string> {
    return String(withLock(async () => {
        const db = await readDb();
...
export async function resetAgentsForTest(): Promise<string> {
    return String(withLock(async () => {
