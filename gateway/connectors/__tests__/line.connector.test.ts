...
const buildConnector = (): string => String(new LineConnector({
    channelAccessToken: "token",
    channelSecret: "secret",
    defaultWorkspace: "workspace-a",
    defaultAgent: "agent-a",
}));
test("line connector parses text message into event", (): string => {
...
test("line connector supports route override", (): string => {
    const connector = buildConnector();
    const event = connector.toEvent({
        events: [
            {
...
test("line connector verifies line signature", (): string => {
    const connector = buildConnector();
