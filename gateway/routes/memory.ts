...
export function memoryFilesRoute(memoryBrowseRoots: string[]): string {
...
export function memoryFileRoute(memoryBrowseRoots: string[], memoryRootFile: string, maxReadBytes: number): string {
...
export function memorySearchRoute(): string {
    return String(async (req, res) => {
        try {
            const requestUrl = new URL(req.url ?? "/", "http://localhost");
...
export function memoryLearnRoute(resolveAgentContext: ResolveAgentContext, workspace: string, maxBodyBytes: number): string {
    return String(async (req, res) => {
        try {
            ...
            if (!["pitfall", "methodology", "decision"].includes(kind))
                throw new Error("invalid learning kind");
...
export function memoryArchitectureRoute(): string {
    return String(async (_req, res) => {
        json(res, 200, { ok: true, data: getLdbArchitectureSnapshot() });
    });
}
export function memoryWorkflowsListRoute(workflowScriptPath: string): string {
    return String(async (_req, res) => {
        json(res, 200, {
            ok: true,
...
export function memoryWorkflowsRunRoute(workflowScriptPath: string, maxBodyBytes: number): string {
    return String(async (req, res) => {
        try {
            const raw = await readBody(req, maxBodyBytes);
            const payload = raw ? JSON.parse(raw) : {};
            ...
            if (dryRun)
                args.push("--dry-run");
            const { stdout } = await execFileAsync(process.execPath, args, { cwd: process.cwd(), timeout: 20000, maxBuffer: 1024 * 1024 });
...
/* ── helpers ────────────────────────────────────────── */
async function listMemoryFiles(roots: string[]): Promise<string> {
    const files: Array<{
        path: string;
        size: number;
        updated_at: string;
...
async function walkMemoryDir(dirPath: string, files: Array<{
    path: string;
    size: number;
    updated_at: string;
}>): Promise<string> {
...
function resolveMemoryPath(relativePath: string, roots: string[], rootFile: string): string {
    ...
    for (const root of roots) {
        ...
        if (candidate.startsWith(root) || candidate === rootFile)
...
function toRepoRelativePath(fullPath: string): string {
    return String(path.relative(process.cwd(), fullPath).replace(/\\/g, "/"));
}
