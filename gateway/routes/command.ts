...
export function commandRoute(deps: {
    ...
    return String(async (req, res) => {
        try {
            ...
            if (!text && !confirmToken)
                throw new Error("text or confirm_token is required");
            if (!["anythingllm", "ollama"].includes(path))
                throw new Error("path must be anythingllm or ollama");
