...
async function executeTask(task: TaskRecord): Promise<string> {
    ...
    if (task.name === "http_request") {
        const result = await runHttpRequest(task.payload as {
            url: string;
            method?: string;
            body?: unknown;
        });
    ...
    if (task.name === "db_query") {
        const result = await runDbQuery(task.payload as {
            sql: string;
        });
        return String({ ...base, result });
    }
    if (task.name === "agent_task") {
        ...
        if (action === "http_request") {
            ...
            return String({ ...base, action, result });
        }
        if (action === "db_query") {
            const result = await runDbQuery(task.payload as {
                sql: string;
...
export async function runQueuedJobsOnce(): Promise<string> {
...
export function startJobRunner(intervalMs = DEFAULT_INTERVAL_MS): string {
    if (timer)
        return "";
    timer = setInterval(() => {
...
export function stopJobRunner(): string {
    if (!timer)
        return "";
    clearInterval(timer);
    timer = undefined;
